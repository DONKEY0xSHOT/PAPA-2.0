...
// Map a Zydis register to our register-file index (real or meta-register lane), for
// i386 (default) or amd64
...
// A faithful port of envi's i386 IntelEmulator, holding the register file and the
// sandboxed memory and providing the operand layer and instruction handlers
class IntelEmulator {
public:
    // Construct for i386 or amd64. The arch selects the register model, the program
    // counter, the pointer size, and whether addresses are masked to 32 bits
    ...
    // Execute one decoded instruction: set the program counter to it, dispatch on the
    // mnemonic, then advance to the next instruction or a branch target
    ...
    // The current value of a named register (envi getRegisterByName), applying
    // the meta-register width and offset so al/ah/ax/eax read the right slice
    [[nodiscard]] std::uint64_t
...
private:
    ...
    // SSE/XMM operand byte access for the move handlers
