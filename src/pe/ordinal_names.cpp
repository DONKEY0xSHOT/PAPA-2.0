...
// oleaut32.dll
constexpr std::array<OrdEntry, 398> kOleaut32{{
    {2, "SysAllocString"},
    {3, "SysReAllocString"},
    {4, "SysAllocStringLen"},
...
[[nodiscard]] std::optional<std::string_view> find_entry(
    const OrdEntry* entries, std::size_t count, std::uint32_t ordinal) noexcept {
    for (std::size_t i = 0; i < count; ++i) {
        if (entries[i].ordinal == ordinal) { return entries[i].name; }
    }
    return std::nullopt;
}
...
std::optional<std::string_view> lookup_ordinal_name(
    std::string_view dll, std::uint32_t ordinal) noexcept {
    if (dll == "ws2_32" || dll == "wsock32") {
        return find_entry(kWs2_32.data(), kWs2_32.size(), ordinal);
    }
    if (dll == "oleaut32") {
        return find_entry(kOleaut32.data(), kOleaut32.size(), ordinal);
    }
    return std::nullopt;
}
